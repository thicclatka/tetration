...
The **query engine** (`src/query/engine/`) turns a validated JSON [`QueryDocument`](../src/query/types/document.rs) into a [`QueryResponse`](../src/query/types/response.rs): catalog resolution, a chunk-level **read plan**, and optionally mmap-backed **`f32`** decode plus **`operation`** aggregates.
...
| **chunk_decode**  | `chunk_decode.rs`  | Mmap slice bounds, codec decode (raw **0**, zstd **1**), `visit_planned_chunk`, scatter into logical buffers.                                                                                                                                                        |
| **materialize**   | `materialize.rs`   | Full/capped materialize, `spill_read_plan_f32_le`, scalar fold entry (`fold_read_plan_scalar_operation`).                                                                                                                                                            |
...
| **operations**    | `operations.rs`    | `build_execution_preview` — routes by memory strategy to fold, partial fold, capped materialize, or mmap spill.                                                                                                                                                      |
...
Host RAM is read best-effort via **`utils::host_memory`** (Linux `MemAvailable`, macOS free+inactive pages). Resolved budget and probe results appear on **`execution.memory_budget_bytes`**, **`execution.host_available_ram_bytes`**, and **`execution.logical_selection_f32_bytes`** in the response.
...
- **`materialize_read_plan_f32_le_parallel`** / **`materialize_read_plan_f32_le_into_parallel`** — same APIs; Rayon over planned chunks (raw and zstd). **`build_execution_preview`** uses parallel decode when the read plan touches more than one chunk and materialization is required.
- **`planned_chunk_mmap_slices`** — zero-copy raw codec slices only (no zstd).
- **`spill_read_plan_f32_le`** — full logical decode to a caller-owned file path (used by **`mmap_spill`** strategy).
...
| ---------------- | -------------- | -------------------------------- | ------------------------------------------------------------ |
| **`sum`**        | `sum`          | `operation_sum`                  | `operation_reduced_sum` + `operation_reduced_shape`          |
| **`mean`**       | `mean`         | `operation_mean`                 | `operation_reduced_mean` + `operation_reduced_shape`         |
| **`min`**        | `min`          | `operation_min`                  | `operation_reduced_min` + `operation_reduced_shape`          |
| **`max`**        | `max`          | `operation_max`                  | `operation_reduced_max` + `operation_reduced_shape`          |
| **`count`**      | `count`        | `operation_element_count`        | `operation_reduced_count` + `operation_reduced_shape`        |
| **`var`**        | `var`          | `operation_var`                  | `operation_reduced_var` + `operation_reduced_shape`          |
| **`std`**        | `std`          | `operation_std`                  | `operation_reduced_std` + `operation_reduced_shape`          |
| **`product`**    | `product`      | `operation_product`              | `operation_reduced_product` + `operation_reduced_shape`        |
| **`norm_l1`**    | `norm_l1`      | `operation_norm_l1`              | `operation_reduced_norm_l1` + `operation_reduced_shape`      |
...
{
  "dataset": "temperature",
  "output": { "preferred": { "spill_array": { "handle": "/tmp/temp_slice.bin" } } }
}
```
...
**Done:** `sum`, `mean`, `min`, `max`, `count`, `var`, `std`, `product`, `norm_l1`, `norm_l2`, `all_finite`, `any_nan` (scalar + partial axes).
...
When adding an op, update this table, [`Operation`](../src/query/types/document.rs), `validate_query` / `document.rs`, `reduction.rs` / `operations.rs` / `partial_fold.rs`, and (if tier **A**) `materialize.rs` fold support.
...
| **Spill path**      | `output.preferred.spill_array.handle` in query JSON      | Host path chosen by caller; no allowlist policy yet (see hardening roadmap)                               |
...
7. **No `.tet` path in query JSON** — Opening a `.tet` file uses the **host** path (`--tet`, API argument), not a field inside the query document. Spill **output** paths *are* in JSON today; deployments should allowlist or rewrite them.
...
- Query planning, materialize, operations, and memory budget: [`tests/query.rs`](../tests/query.rs).
