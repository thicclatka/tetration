...
| **`fold/`**        | `fold_policy.rs`, `linear_scan.rs`, `shared.rs`, `reduction.rs`, `variance_simd.rs`, `parallel/` (`workers`, `preview`, `merge`, `scalar`, `partial`), `partial_fold.rs`, … | `FoldIoPolicy` / I/O regime; out-of-core **linear scan**; `FoldPlanOutcome`, `ReductionKind`; SIMD bulk **`f32`** sum/sumsq + min/max; parallel + partial-axis streaming folds. |
