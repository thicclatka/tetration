...
- **`tet query [QUERY]`** — `QUERY` is a path to `.json`, inline JSON, `-`, or omit for stdin. Parses JSON with the same validation as the library; stdout format is **`--format full|json|stats|plan|quiet`** (default **`full`**; **`-q`** = quiet one-liner). **Without `-t`**, **`plan_query_empty`** echoes the validated plan only. **With `-t path.tet`**, **`plan_query_with_tet_mmap`** adds **`catalog`** and **`read_plan`**. **With `-t`** and **`-x` / `--execute`**, the engine attaches **`execution`** (previews, budget, **`operation_*`**, or **`mmap_spill`** for **`"spill": "path"`**). Preview cap: **`--preview N`** (alias **`--preview-f32`**; default **64** for `full`/`json`, **0** for `stats`/`quiet` when omitted); **`0`** with an **`operation`** or **`spill`** skips preview arrays but still runs aggregates or export. The library still returns a full [`QueryResponse`](https://docs.rs/tetration/latest/tetration/struct.QueryResponse.html); embedders can call **`format_query_response`** for the same stdout modes.
- **`tet qhist`** (`hist` alias) — Platform query history (`query_history.jsonl`, not in `.tet`): **`list`** (filters, **`--json`**), **`run N`** replay, **`--clear`**. See [`GETTING_STARTED.md`](GETTING_STARTED.md#cli-query-history-tet-qhist).
...
cargo run -- query '{"dataset":"f32"}' -t data.tet --format plan
cargo run -- query '{"dataset":"f32","spill":"slice.bin"}' -t data.tet -x --format stats
cargo run -- qhist list --grep mean
cargo run -- qhist run 1 -q
