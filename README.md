...
The on-disk layout stays binary and mmap-oriented; JSON sits **above** it as a **stable, versioned interchange** for “what to read or compute,” not as the storage encoding of tensor bytes. A query document might carry `layout_version`, `dataset`, `selection` (per-axis start/stop/step), optional `operation` (**`sum`**, **`mean`**, **`min`**, **`max`**, **`count`** with **`axes: []`** for a scalar over the whole logical selection, or **`axes: ["0",…]`** with decimal dimension indices for partial reductions), and `output` hints (inline JSON stats vs spill to a new array handle). The **`src/query/engine/`** module validates against a mmap’d catalog, builds a **`ReadPlan`**, and can decode **`f32`** tiles (raw or zstd) in **logical row-major** selection order; the **`tet`** CLI exposes this via **`--tet`** and **`--execute`**. Multi-chunk **`--execute`** paths decode chunks in parallel (Rayon); single-chunk plans stay sequential. Query JSON is **not executable**—hosts should still cap input size and treat responses as data when embedding in shells, HTML, or logs ([JSON security](docs/query_engine.md#json-security-input-and-output)).
...
- **`tet query`** — Load a query document from **`--file` / `-f`** (path to JSON) or from **standard input** (omit `-f`, or pass `-`). The tool parses JSON, runs the same validation rules as the library, and prints a **pretty-printed JSON** [`QueryResponse`](https://docs.rs/tetration/latest/tetration/query/struct.QueryResponse.html). **Without `--tet`**, the library’s **`plan_query_empty`** echoes the validated document only (no catalog or mmap). **With `--tet path.tet`**, **`plan_query_with_tet_mmap`** adds **`catalog`** (dataset resolution) and **`read_plan`** (per-chunk payload regions and selection geometry). **With `--tet`** and **`--execute`**, the engine memory-maps the file, decodes planned **`f32`** chunk payloads (**codec 0** raw or **codec 1** zstd) into **logical row-major** selection order, and attaches **`execution`**: capped **`f32_preview`** (default **64**; **`--preview-f32 0`** with an **`operation`** skips preview floats while still aggregating). When an **`operation`** is set, **`operation_element_count`** and scalar **`operation_*`** fields (`axes: []`, e.g. **`operation_sum`**, **`operation_min`**) or **`operation_reduced_*`** (decimal axis indices) summarize the **full** logical tensor; scalar ops use a single-pass fold without allocating the full buffer. The preview cap does not truncate those stats.
...
Query JSON follows the shapes described above (`dataset`, optional `layout_version`, `selection` as per-axis `{ start, stop, step }`, optional `operation` such as `{ "min": { "axes": [] } }`, `{ "mean": { "axes": [] } }`, or `{ "sum": { "axes": ["0"] } }`, optional `output.preferred`). Library entrypoints: **`plan_query_empty`**, **`plan_query_with_tet_mmap`**, **`materialize_read_plan_f32_le`**, **`materialize_read_plan_f32_le_into`**, **`materialize_read_plan_f32_le_parallel`**, **`materialize_read_plan_f32_le_into_parallel`** (see [`docs/query_engine.md`](docs/query_engine.md)).
