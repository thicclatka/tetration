...
**Non-goals (v1):** SQL-on-files, arbitrary codec plugins, GPU codecs in the file format. **Phase 10 (experimental, [PR #12](https://github.com/thicclatka/tetration/pull/12)):** optional Metal/CUDA/ROCm for tier-A/B **`f32`**/**`f16`** — dense or streaming device fold; CPU streaming stays the practical default on large unified-memory hosts. See [`GETTING_STARTED.md`](GETTING_STARTED.md#phase-10--gpu-experimental) and [`docs/query_engine.md`](docs/query_engine.md#phase-10--optional-gpu-experimental). **Next:** Phase 11 Python wheels + narrow C ABI; the layout spec is the portable floor.
