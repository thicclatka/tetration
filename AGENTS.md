...
| Phase 6 (CLI & query UX)                                                    | **Done** — focused query stdout, `tet qhist` list/run/filter (`hist` alias), flat query **JSON + TOML**, `tet info` table/filters/`--json`; spawn-`tet` smoke in `src/tests/cli_info.rs`                                                                                                                                                                                                                                                                                                                            |
...
- **CLI / query UX (Phase 6+):** optional line-oriented query profile; optional query `preview` stdout table; **`qhist run --plan`** on saved op rows (needs plan-only path without op gate). **TOML:** [`parse_query_toml`](src/query/document_toml.rs), CLI `.toml` paths + auto-detect.
