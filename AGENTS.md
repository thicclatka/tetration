...
| Phase 6 (CLI & query UX)                                                    | **Done** — focused query stdout, `tet qhist` list/run/filter (`hist` alias), flat query JSON, `tet info` table/filters/`--json`; **next:** spawn-`tet` tests, optional TOML front-end                                                                                                                                                          |
...
| `tests/cli_info.rs`         | `tet info` table, `--json`, filters, spawn `tet` smoke                                                                      |
...
- **CLI / query UX (Phase 6+):** optional TOML/line-oriented query front-end, spawn-`tet` integration tests (beyond `cli_info` smoke), optional query `preview` stdout table; **`qhist run --plan`** on saved op rows (needs plan-only path without op gate).
