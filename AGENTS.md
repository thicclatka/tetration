...
| Phase 8 (dtypes & file health)                                              | **In progress** — [`verify`](src/verify/mod.rs) module: **`tet verify`**, chunk decode checks, recommendations; **next:** additional wire dtypes (`u8`, …)                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `tet verify <path.tet>`                               | Layout + chunk integrity; recommendations on failure; `--json` / `-q` ([`verify_tet_file`](src/verify/mod.rs)).                                                 |
...
- **Dtypes & verify (Phase 8):** **`tet verify`** baseline shipped; additional dtypes (`u8`, …) still open — see [`GETTING_STARTED.md` — Phase 8](GETTING_STARTED.md#phase-8--dtypes--file-health-next).
