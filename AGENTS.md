...
| Phase 7 (metadata & history)                                                | **In progress** — history footer + convert events; **`TetWriterSession`** / **`TetFile`** / **`execute_query_*`** + crate examples; **next:** wire file/dataset metadata, append-to-file session, streaming writer session                                                                                                                                                                                                                                                                                                                      |
...
- **Metadata (Phase 7):** `THST` footer `metadata` JSON; **`tet convert`** imports HDF5/NetCDF attrs + NC `dim_names`; **`catalog::session`**, **`query::execute_query_*`**; **next:** append/streaming session, coordinate labels, Zarr attrs.
