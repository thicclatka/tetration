...
| Phase 8 (dtypes & file health)                                              | **Done** — `tet verify` quick scan (128-chunk decode sample; **`--deep`** = all chunks) / `tet repair`; wire dtypes **`f32`–`u64`** (tags `1`–`10`, incl. **`f16`**); booleans → **`u8`**; integer bulk SIMD (`i32`/`u8` sum/var/min-max)                                                                                                                                                                                                                                                                                                         |
...
- **`query`:** Flat JSON wire in **`document_wire.rs`**; limits + **`validate_query`** in **`document.rs`** (`QueryLimits`). Wire types in **`query/types/`**. Submodules under **`src/query/`**:
  ...
  | **`fold/`**        | `fold_policy` (I/O regime), `linear_scan` (out-of-core byte-stream fold), `reduction`, `variance_simd`, `parallel/` (`workers`, `preview`, `merge`, `scalar`, `partial`), `shared` (`FoldPlanOutcome`), `partial_fold`, `partial_geometry` |
...
- **Phase 8:** closed — verify/repair + dtypes through `i16`; optional follow-ups (`u32`, `f16`, integer SIMD) documented in [`GETTING_STARTED.md` — Phase 8](GETTING_STARTED.md#phase-8--dtypes--file-health-done).
