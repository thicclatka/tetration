...
| Phase 6 (CLI & query UX)                                                    | **Partial** — focused stdout, `tet history` list/run/filter, flat query JSON wire (`document_wire.rs`), `--format plan`; **next:** spawn-`tet` tests, optional TOML front-end, preview table                                                                                                                                                    |
...
| `tet history` / `history list`                        | Compact table (`-n`, `--all`, **`--json`**); filters **`--dataset`**, **`--tet`**, **`--mode x\|p`**, **`--grep`**; indices **1** = newest in filtered view. |
| `tet history run N`                                   | Re-run saved row (**1** = newest in filtered list); **`-t`** / **`-x`** / **`--plan`** override; stdout from current **`--format`** / **`-q`**.              |
| `tet history --clear`                                 | Remove `query_history.jsonl`.                                                                                                                                |
...
- **CLI / query UX (Phase 6):** optional TOML/line-oriented query front-end, spawn-`tet` integration tests, optional `preview` stdout table. Flat JSON wire is v1 (`document_wire.rs`; nested `"operation"` rejected).
