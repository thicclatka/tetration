...
| Phase 7 (metadata & history)                                                | **Baseline done** — footer `metadata` JSON; **`TetWriterSession`** (create, **`open_append`**, streaming **`commit_with_fill`**), **`TetFile`**, **`execute_query_*`**; convert imports attrs / `dim_names` / inline **`coords`**; **next:** history v2, footer size policy, session `coords` on write                                                                                                                                                                                                                                            |
...
- **Metadata (Phase 7):** `THST` footer `metadata` JSON; **`tet convert`** imports HDF5/NetCDF/Zarr attrs + NC `dim_names`; **`catalog::session`** (append + streaming), **`query::execute_query_*`**; **next:** coordinate labels, history v2, footer spill.
