...
| Phase 8 (dtypes & file health)                                              | **Done** — `tet verify` / `tet repair` (`--deep`, dataset tensor-byte cross-check); wire dtypes **`f32`–`i16`** (tags `1`–`7`); booleans → **`u8`**; optional stretch: `u32`/`f16`, integer SIMD                                                                                                                                                                                                                                                                                                                                                  |
...
- **`query`:** Flat JSON wire in **`document_wire.rs`**; limits + **`validate_query`** in **`document.rs`** (`QueryLimits`). Wire types in **`query/types/`**. Submodules under **`src/query/`**:
  ...
  | **`materialize/`** | f32/f64/i32/i64/u8/u16/i16 materialize, `parallel`, `int`, `stats` (tier-C)                                                                                                               |
