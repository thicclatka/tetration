...
| Phase 8 (dtypes & file health)                                              | **In progress** — file health **done**; wire dtypes **`f32`–`i16`** (tags `1`–`7`, incl. `u8`/`u16`/`i16`); booleans import as **`u8`**; **next:** wider ints (`u32`, `f16`, …)                                                                                                                                                                                                                                                                                                                                                                |
...
| `tet verify <path.tet>`                               | Layout + chunk integrity; recommendations + `tet repair …` hints; `--json` / `-q` / `--repair` ([`verify_tet_file`](src/verify/mod.rs)).                     |
...
- **Dtypes (Phase 8):** file health shipped; **`u8`/`u16`/`i16`** (tags `5`–`7`) shipped; booleans import as **`u8`**; wider types (`u32`, `f16`, …) open — see [`GETTING_STARTED.md` — Phase 8](GETTING_STARTED.md#phase-8--dtypes--file-health-in-progress).
