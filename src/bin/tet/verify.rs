...
use tetration::verify::{
    format_verify_json, format_verify_quiet, format_verify_text, verify_tet_file,
};
...
pub(crate) fn run_verify(opts: &VerifyRunOpts) -> Result<(), String> {
    let mut report = verify_tet_file(&opts.path).map_err(cli_error)?;
    if opts.repair && !report.ok {
        ...
        report = verify_tet_file(&opts.path).map_err(cli_error)?;
    }
